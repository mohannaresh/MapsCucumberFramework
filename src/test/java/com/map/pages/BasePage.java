...
import com.maps.utils.GlobalParams;
import com.maps.utils.TestUtils;
...
public class BasePage {
    ...
    public MobileElement andScrollToElementUsingUiScrollable(String childLocAttr, String childLocValue) {
        return (MobileElement) ((FindsByAndroidUIAutomator) driver).findElementByAndroidUIAutomator("new UiScrollable(new UiSelector()" + ".scrollable(true)).scrollIntoView(" + "new UiSelector()." + childLocAttr + "(\"" + childLocValue + "\"));");
    }
...
//        scrollObject.put("toVisible", "sdfnjksdnfkld");
    ...
    public void selectAddressFromAutoSuggestionTextView(String addressText) throws Exception {
        switch (new GlobalParams().getPlatformName()) {
            case "Android":
                By element = By.xpath("//*[@text=" + addressText + "]");
                scrollToElement(element, "down");
                click(element, "Choose entered address from auto suggestion text view");
                break;
            case "iOS":
                MobileElement mobileElement = (MobileElement) driver.findElement(By.xpath("//*[@text=" + addressText + "]"));
                iOSScrollToElementUsingMobileScroll(mobileElement);
                click(mobileElement, "Choose entered address from auto suggestion text view");
                break;
            default:
                throw new IllegalStateException("Unexpected value: " + new GlobalParams().getPlatformName());
        }
    ...
    public List<WebElement> getListOfElements(By element) {
        return (List<WebElement>) driver.findElements(element);
    }

    public List<String> getListOfElementsText(List<WebElement> inputList) {
        List<String> list = new ArrayList<String>();
        for (int i = 0; i < inputList.size(); i++) {
            list.add(inputList.get(i).getText());
        }
        return list;
    }

    public void clickElementBasedOnText(List<WebElement> inputList, String inputString) {
        for (int i = 0; i < inputList.size(); i++) {
            if (inputList.get(i).getText().toLowerCase().equalsIgnoreCase(inputString.toLowerCase())) {
                click((MobileElement) inputList.get(i));
            }
        }
    }
}
